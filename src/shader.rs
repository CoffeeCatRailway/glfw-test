...
impl Shader {
	...
	pub unsafe fn getAttribLocation(&self, name: &CStr) -> GLint {
		unsafe { gl::GetAttribLocation(self.id, name.as_ptr()) }
	}

	// Uniforms
	pub unsafe fn setUniform1i(&self, name: &CStr, value: i32) {
		unsafe { gl::Uniform1i(gl::GetUniformLocation(self.id, name.as_ptr()), value); }
	}

	pub unsafe fn setUniform1ui(&self, name: &CStr, value: u32) {
		unsafe { gl::Uniform1ui(gl::GetUniformLocation(self.id, name.as_ptr()), value); }
	}

	pub unsafe fn setUniform1f(&self, name: &CStr, value: f32) {
		unsafe { gl::Uniform1f(gl::GetUniformLocation(self.id, name.as_ptr()), value); }
	}

	pub unsafe fn setUniform2fv(&self, name: &CStr, value: &Vector2<f32>) {
		unsafe { self.setUniform2f(name, value.x, value.y); }
	}

	pub unsafe fn setUniform2f(&self, name: &CStr, x: f32, y: f32) {
		unsafe { gl::Uniform2f(gl::GetUniformLocation(self.id, name.as_ptr()), x, y); }
	}

	pub unsafe fn setUniform3fv(&self, name: &CStr, value: &Vector3<f32>) {
		unsafe { self.setUniform3f(name, value.x, value.y, value.z); }
	}

	pub unsafe fn setUniform3f(&self, name: &CStr, x: f32, y: f32, z: f32) {
		unsafe { gl::Uniform3f(gl::GetUniformLocation(self.id, name.as_ptr()), x, y, z); }
	}

	pub unsafe fn setUniform4fv(&self, name: &CStr, value: &Vector4<f32>) {
		unsafe { self.setUniform4f(name, value.x, value.y, value.z, value.w); }
	}

	pub unsafe fn setUniform4f(&self, name: &CStr, x: f32, y: f32, z: f32, w: f32) {
		unsafe { gl::Uniform4f(gl::GetUniformLocation(self.id, name.as_ptr()), x, y, z, w); }
	}

	pub unsafe fn setMatrix4f(&self, name: &CStr, mat: &Matrix4<f32>) {
	...
	unsafe fn compileShader(&self, path: &str, shaderType: GLenum) -> GLuint {
		...
		file.read_to_string(&mut code).expect("Failed to read vertex shader");
		...
		unsafe {
			...
			} else { println!("Shader compilation successful: {}-{}", shader, path); }
	...
	unsafe fn checkLinkStatus(&self, id: GLuint) {
		unsafe {
			...
			} else { println!("Shader linking successful: {}", id); }
		}
	}
}
